...
@Component({
  selector: 'app-products-by-invoice',
  templateUrl: './products-by-invoice.component.html'
})
export class ProductsByInvoiceComponent implements OnInit {
  ...
  getProductsReceptionByInvoice(){
  	this.spinner.show();
    this.productReceptionService.getByInvoice(parseInt(this.route.snapshot.paramMap.get('id'))).toPromise().then(
      response => {
        if(response!=undefined && response.data){
          this.spinner.hide();
          this.factura=response.data.invoice;
          this.folder=response.data.folder;
          this.folderClosed=response.data.folder.status=="cerrada"?true:false;
        }
      }).catch( error => {
        this.spinner.hide();
    ...
    getQuantityReported(i:number){
      const productReported=this.factura.products_received_by_version[i].product_reception.product_reception_by_version.find(element=>{
        if(this.factura.products_received_by_version[i].id_product_reception==element.id_product_reception&&this.factura.products_received_by_version[i].id_version_product==element.id_version_product){
          return element;
        }
      });
      return productReported?productReported.quantity_reported:'Sin cantidad registrada';
    }
    getQuantityReceived(i:number){
      const productReceived=this.factura.products_received_by_version[i].product_reception.product_reception_by_version.find(element=>{
        if(this.factura.products_received_by_version[i].id_product_reception==element.id_product_reception&&this.factura.products_received_by_version[i].id_version_product==element.id_version_product){
          return element;
        }
      });
      return productReceived?productReceived.quantity_reported:'Sin cantidad registrada';
    }
    updateReceived(j:number,id_product_reception:number,version_product:any,quantity_received:number,quantity_process:number){
      ...
      response => {
        if(response!=undefined && response.data){
          this.spinner.hide();
          this.factura.products_received_by_version=this.factura.products_received_by_version.map(element=>{
    ...
    getPreviewBatchReceived(productReceiveds:any[]){
      this.spinner.show();
      this.productReceptionService.getPreviewBatchReceived(productReceiveds).toPromise().then(
        response => {
          if(response!=undefined && response.data){
            this.notificationService.showSuccess('Operación realiza exitosamente',response.message)
            this.spinner.hide();
    ...
    confirmPreviewBatchReceived(){
      ...
      let descriptions='';
      let productReceptions=this.factura.products_received_by_version.map(element=>{
        const i=this.factura.products_received_by_version.indexOf(element);
    ...
    if(descriptions.length>0){
      Swal.fire({
        ...
        if (result.value) {
          this.getPreviewBatchReceived(productReceptions);
        }
      })
    }else{
      this.getPreviewBatchReceived(productReceptions);
    ...
    saveBatchReceived(productReceiveds:any[]){
      this.spinner.show();
      this.productReceptionService.batchReceived(this.factura.id_carpeta,this.closeFolder,productReceiveds).toPromise().then(
        response => {
          if(response!=undefined && response.data){
            this.spinner.hide();
            ...
            for(const element of elements){
              this.factura.products_received_by_version=this.factura.products_received_by_version.map(elementMap=>{
    ...
    confirmSaveElements(){
      if(this.modalRef)
        this.modalRef.close();
        let descriptions='';
        let productReceiveds=this.factura.products_received_by_version.map(element=>{
          const i=this.factura.products_received_by_version.indexOf(element);
        if(element.quantity_process>0 && element.quantity_process+element.quantity_received>this.getQuantityReceived(i)){
            descriptions=descriptions+element.product_reception.product.descriptions+"<br>";
          }
        ...
        productReceiveds=productReceiveds.filter(element=>{
        ...
        if(descriptions.length>0){
          Swal.fire({
            ...
            if (result.value) {
              this.saveBatchReceived(productReceiveds);
            }
          })
        }else{
          this.saveBatchReceived(productReceiveds);
      ...
      /*generacion de reporte en excel*/
        generateReport(){
          this.data=new Array();
          for(let productReceived of this.factura.products_received_by_version){
            const i=this.factura.products_received_by_version.indexOf(productReceived);
            this.data.push({
              ...
              factura:this.factura?this.factura.folioNum:'Sin factura registrada'
            });
          }     
