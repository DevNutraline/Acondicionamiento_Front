...
export class SupplieService extends BaseService{
  private servicio = 'insumo/';
  ...
  get token(): string {
    ...
    create(supplie_data: any): Observable<any> {
        let user_create_url = this.servicio + 'store'
        return this.post(this.http, user_create_url, supplie_data);
    }
    ...
    updateElement(id, supplie_data: any): Observable <any> {
        let update_user_url = this.servicio + `update/${id}`
        return this.post(this.http,update_user_url, supplie_data);
    }
    errorHandl(error) {
      let errorMessage = '';
