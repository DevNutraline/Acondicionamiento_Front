...
export class LotService extends BaseService{
  ...
  getAllWithProduct(): Observable <any>{
    let url = this.servicio +'withproduct'
        return this.http.get(this.url + url,{
          headers:{
            'Content-type': 'application/json',
            Authorization: `Bearer ${this.token}`
    ...
    addPermissions(id, data: any): Observable <any> {
        let addpermissions_url = this.servicio + `addpermissions/${id}`
        return this.post(this.http,addpermissions_url, data);
    }
    errorHandl(error) {
      let errorMessage = '';
