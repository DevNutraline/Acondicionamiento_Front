...
export class SupplieInventoryService extends BaseService{
  private servicio = 'inventarioinsumo/';
  ...
  get token(): string {
    ...
    getMovementHistorie(): Observable <any> {
        let get_all_users_url = this.servicio + 'movementhistorie/all'
        return this.http.get(this.url + get_all_users_url,{
          headers:{'Content-type': 'application/json',
          Authorization: `Bearer ${this.token}`
        }
